    ...
    class rtTPTField
    {
        ...
        private function setArgument($key, $attr){
	        ...
	        if(!$this->value){
		        global $rtTPT;
		        $post_id = get_the_ID();
		        if(!$rtTPT->meta_exist($post_id, $this->name)){
        ...
        {
        	global $rtTPT;
	        $icons = $rtTPT->getFontAwsomeIconList();
	        $slt = ($this->value ? "<i class='fa {$this->value}'></i>" : null);
            $h = null;
	        $h .= "<div class='icon-list-wrapper'>";
		        $h .= "<div class='selected-icon-wrap'><span class='selected-icon'>{$slt}</span> <input type='hidden' name='{$this->name}' value='$this->value' /> <div class='toolbar hide'><span class='show-icons'>Show icons</span><span class='hide-icons'>Hide icons</span></div></div>";
	            $h .= "<div class='icon-list-wrap rt-hidden'>";
	                foreach ($icons as $key => $icon) {
	                    $Islt = $key == $this->value ? " selected" : null;
		                $h .= "<span class='icon{$Islt}' data-id='{$key}' ><i class='fa {$key}'></i></span>";
	                }
	            $h .= "</div>";
            $h .= "</div>";
            return $h;
